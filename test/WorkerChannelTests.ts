...
describe('WorkerChannel', () => {
    ...
    it('does not init for Node.js v14.x and v2 compatability = true', () => {
        ...
        if (version.split('.')[0] === 'v14') {
            ...
            expect(() => stream.addTestMessage(initMessage)).to.throw(
                `Incompatible Node.js version (${process.version}). The version of the Azure Functions runtime you are using (v2) supports Node.js v8.x and v10.x. Refer to our documentation to see the Node.js versions supported by each version of Azure Functions: https://aka.ms/functions-node-versions`
            );
        }
    });
    ...
    it('invokes function in V2 compat mode', () => {
        ...
        expect(JSON.stringify(actualInvocationRequest.triggerMetadata!.$request)).to.equal(
            JSON.stringify(httpInputData.data)
        );
        expect(JSON.stringify(actualInvocationRequest.triggerMetadata!.req)).to.equal(
            JSON.stringify(httpInputData.data)
        );
    });
