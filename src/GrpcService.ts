...
function GetGrpcClientConstructor(): GrpcClientConstructor {
    const reflectionObject = protobuf.Root.fromJSON(jsonModule as protobuf.INamespace);
    const rpcs = grpc.loadObject(reflectionObject, { enumsAsStrings: false, protobufjsVersion: 6 });
...
export function CreateGrpcEventStream(connection: string, grpcMaxMessageLength: number): IEventStream {
    const GrpcClient = GetGrpcClientConstructor();
    const clientOptions = {
        'grpc.max_send_message_length': grpcMaxMessageLength,
        'grpc.max_receive_message_length': grpcMaxMessageLength,
    };
    const client = new GrpcClient(connection, grpc.credentials.createInsecure(), clientOptions);
